...
> 图1 比较各种量化方法。其中权重值是从GoogLeNet的第二个![](http://latex.codecogs.com/gif.latex?3%5Ctimes3) 卷积层获取。每种量化方式都将该分布量化到24级。我们使用$2^{0.5}$作为对数量化的底， 并对线性和对数量化进行优化，尽可能最小化L2归一化对整体激活输出的影响。
...
第一步，我们定义一个量化度量来决定单个权重的重要性(或影响权重的输出质量)。由于较大的权重对于输出质量有较大的影响，我们根据经验定义了重要性![](http://latex.codecogs.com/gif.latex?i_%7B%28n%2C%20m%29%7D)，其为第n个集群的第m个权重的重要性。例如，![](http://latex.codecogs.com/gif.latex?w_%7B%28n%2C%20m%29%7D)与其重要性的大小成一定的比例，比如，![](http://latex.codecogs.com/gif.latex?i_%7B%28n%2Cm%29%7D%20%3D%20w_%7B%28n%2Cm%29%7D%5E2)。
